...
import ProfileIdPanel from "../profile/ProfileIdPanel";
import UserLoginHistory from "./UserLoginHistory";
import {
  Search, Users, UserCircle2, Loader2, ShieldCheck, AlertTriangle,
  CheckCircle, XCircle, Clock, User, Briefcase, ToggleLeft, ToggleRight, Zap, Building2
} from "lucide-react";
...
export default function AdminProfilesPanel() {
  ...
  async function loadProfiles() {
    ...
    if (res.data?.profiles) {
      ...
      setTotalCount(res.data.total || res.data.profiles.length);
      setUniqueCount(res.data.unique || res.data.profiles.length);
    } else {
  ...
  async function handleMergeDuplicates() {
    if (!window.confirm("Merge duplicate profiles? This will keep the oldest record as the master and delete duplicates.")) {
      return;
    }
    setMerging(true);
    const res = await base44.functions.invoke("mergeDuplicateProfiles", {});
    if (res.data?.success) {
      // Reload profiles after merge
      await loadProfiles();
    }
    setMerging(false);
  }
    ...
    <div>
      <div className="space-y-3 mb-4">
        <div className="flex items-center justify-between">
          <div>
            <p className="text-sm font-semibold">{uniqueCount} unique registered profile{uniqueCount !== 1 ? "s" : ""}</p>
            {totalCount > uniqueCount && (
              <p className="text-xs text-amber-600 dark:text-amber-400 mt-1">
                ⚠ {totalCount - uniqueCount} duplicate record{totalCount - uniqueCount !== 1 ? "s" : ""} detected
              </p>
            )}
          </div>
          {totalCount > uniqueCount && (
            <button
              ...
              style={{
                background: "rgba(236,44,163,0.15)",
                color: "#ec2ca3",
                border: "1px solid rgba(236,44,163,0.30)",
                opacity: merging ? 0.5 : 1,
                cursor: merging ? "not-allowed" : "pointer",
              }}
            >
              {merging ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Zap className="w-3.5 h-3.5" />}
              Merge Duplicates
            </button>
          )}
        </div>
      ...
      <div className="flex flex-col xl:flex-row gap-4">
        {/* Left: list */}
        <div className="w-full xl:w-[300px] flex-shrink-0 space-y-3">
          ...
          <div className="space-y-1.5 max-h-[calc(100vh-320px)] overflow-y-auto pr-0.5">
            ...
            {filtered.length === 0 && (
              <div className="text-center py-8">
                ...
                <p className="text-xs" style={{ color: isDark ? "rgba(255,255,255,0.30)" : "rgba(0,0,0,0.35)" }}>No profiles found</p>
              </div>
            )}
          </div>
        </div>
            ...
            <div className="space-y-4">
              <div className="relative">
                ...
                <div className="absolute top-4 right-4">
                  <button
                    ...
                    className="flex items-center gap-1.5 text-xs font-semibold px-3 py-1.5 rounded-xl transition-all"
                    style={{
                      background: selected.employment_status === "Active" ? "rgba(239,68,68,0.15)" : "rgba(16,185,129,0.15)",
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                ...
                <ProfileSection title="Work Details" icon={Briefcase} accentColor="#0ea5e9">
                  <div className="space-y-3">
                    ...
                    <ProfileField label="Employment Status" fieldKey="employment_status" value={selected.employment_status} editable onSave={handleFieldSave} options={EMPLOYMENT_OPTIONS} />
                    <ProfileField label="Onboarding Status" fieldKey="onboarding_status" value={selected.onboarding_status} editable onSave={handleFieldSave} options={ONBOARDING_OPTIONS} />
                  </div>
                </ProfileSection>
              </div>
              ...
              <ProfileSection title="Admin Controls" icon={ShieldCheck} accentColor="#a855f7">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <ProfileField label="Role Type" fieldKey="role_type" value={selected.role_type} editable onSave={handleFieldSave} options={["Staff", "Admin"]} />
                ...
                <div className="mt-4 grid grid-cols-2 gap-2">
                  {[
                    ...
                    >
                      {selected[key] ? <ToggleRight className="w-3.5 h-3.5" /> : <ToggleLeft className="w-3.5 h-3.5" />}
                      {label}
                    </button>
                  ))}
                </div>
              ...
              <ProfileSection title="HubSpot Integration" icon={Building2} accentColor="#ff7a00">
                <div className="space-y-3">
                  <ProfileField label="Owner ID" fieldKey="hubspot_owner_id" value={selected.hubspot_owner_id} editable onSave={handleFieldSave} />
                  <ProfileField label="Owner Name" fieldKey="hubspot_owner_name" value={selected.hubspot_owner_name} editable onSave={handleFieldSave} />
                  <ProfileField label="Owner Email" fieldKey="hubspot_owner_email" value={selected.hubspot_owner_email} editable onSave={handleFieldSave} type="email" />
                </div>
              </ProfileSection>

              <ProfileSection title="Login Activity" icon={Clock} accentColor="#06b6d4">
                <UserLoginHistory userId={selected.auth_user_id} />
              </ProfileSection>
            </div>
          ) : (
